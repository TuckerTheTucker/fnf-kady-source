...
class PlayState extends MusicBeatState
{
	...
	public static var dialogueMusic:FlxSound;
	...
	public static var winning:Bool = false;
	...
	override public function create()
	{
		...
		if (FlxG.save.data.gangUnlock == null)
			FlxG.save.data.gangUnlock = false;
			
		if (FlxG.save.data.balamUnlock == null)
			FlxG.save.data.balamUnlock = false;
			
		if (FlxG.save.data.kitUnlock == null)
			FlxG.save.data.kitUnlock = false;
		
		if (FlxG.save.data.goyUnlock == null)
			FlxG.save.data.goyUnlock = false;
			
		if (FlxG.save.data.fightUnlock == null)
			FlxG.save.data.fightUnlock = false;
		
		switch (storyDifficulty) //to avoid problems, we do this instead
		{
			case 0:
				storyDifficultyText = "SIMPLE.";
			case 1:
				storyDifficultyText = "MID";
			case 2:
				storyDifficultyText = "AWESOME!";
		}
		...
		Application.current.window.title = "Friday Night Funkin': Feline Fiasco! ~ " + curSong + ' - ' + storyDifficultyText;

		// set the camera position to the center of the stage
		camPos.set(FlxG.width / 2, (FlxG.height / 2) + 100);
		...
		if (SONG.song.toLowerCase() == 'secret') //apparently you cant move the x no matter what you do so uhh here we go
		{
	...
	override public function update(elapsed:Float)
	{
		...
		if (!inCutscene) {
			...
			// make sure you're not cheating lol (unless debug mode is enabled/you beat the week)
			if (!isStoryMode || debugMode || FlxG.save.data.weekCompleted)
			{
				...
				if (FlxG.keys.justPressed.SIX && startedSong)
				{
				...
				else
				{
					...
					switch (curStage)
					{
						...
						case 'gummistage':
							...
							getCenterY = char.getMidpoint().y - 75;
					}
			...
			if (health <= 0 && startedCountdown || controls.RESET)
			{
	...
	private function generateSong(dataPath:String):Void
	{
		...
		songDetails = CoolUtil.dashToSpace(SONG.song) + ' - ' + storyDifficultyText;
		...
		dialogueMusic = new FlxSound().loadEmbedded(Paths.music('drumBreak'), true, true);
	...
	override function beatHit()
	{
		...
		if (boyfriend.animation.curAnim.name == 'hey' && boyfriend.animation.curAnim.finished)
			boyfriend.dance(true);
			
		if (gf.animation.curAnim.name == 'cheer' && gf.animation.curAnim.finished)
			gf.dance(true);
		
		if (dadOpponent.animation.curAnim.name == 'hey' && dadOpponent.animation.curAnim.finished)
			dadOpponent.dance(true);
		...
		if ((FlxG.camera.zoom < 1.35 && curBeat % 2 == 0) && (!Init.trueSettings.get('Reduced Movements')))
		{
			switch (SONG.song.toLowerCase())
			{
				case 'rivallife':
		...
		switch (curSong.toLowerCase())
		{
			...
			case 'balam':
				//wind causes the notes to speeeen!!
				if (curBeat % 2 == 1)
					SPEEN(0.1);
			...
			case 'secret':
				//hey stuff but different
				switch (curBeat)
				{
					case 50 | 66 | 82 | 162 | 178 | 194 | 298 | 306 | 314:
						boyfriend.playAnim('hey');
				}
		}
	...
	function endSong():Void
	{
		...
		if (!autoPlayOn && !openedChartingMode)
		{
			if (storyDifficulty == 2 && misses == 0)
			{
				...
				{
					case 'ganghan':
						FlxG.save.data.gangUnlock = true;
						
					case 'balam':
						FlxG.save.data.balamUnlock = true;
						
					case 'keunsilsu':
						FlxG.save.data.kitUnlock = true;
						
					case 'goyangi':
						FlxG.save.data.goyUnlock = true;
	...
	private function startCountdown():Void
	{
		...
		startTimer = new FlxTimer().start(Conductor.crochet / 1000, function(tmr:FlxTimer)
		{
			...
			if (curStage == 'outside')
			{
				stageBuild.jace.animation.play('idle');
				stageBuild.tyler.animation.play('idle');
				stageBuild.apple.animation.play('idle');
			}
	...
	function SPEEN(time:Float)
	{
		for (strumline in strumLines)
		{
			FlxTween.tween(strumline.receptors.members[0], {angle: strumline.receptors.members[0].angle + 360}, time, {ease: FlxEase.linear});
			FlxTween.tween(strumline.receptors.members[1], {angle: strumline.receptors.members[0].angle + 360}, time, {ease: FlxEase.linear});
			FlxTween.tween(strumline.receptors.members[2], {angle: strumline.receptors.members[0].angle + 360}, time, {ease: FlxEase.linear});
			FlxTween.tween(strumline.receptors.members[3], {angle: strumline.receptors.members[0].angle + 360}, time, {ease: FlxEase.linear});
		}
	}

	function UNSPEEN()
	{
		for (strumline in strumLines) //another fix for a glitch where changing windows mid speen caused the notes angle to be stuck...this time without needing to restart
		{
			if (strumline.receptors.members[0].angle != 0)
			{
				strumline.receptors.members[0].angle = 0;
				strumline.receptors.members[1].angle = 0;
				strumline.receptors.members[2].angle = 0;
				strumline.receptors.members[3].angle = 0;
			}
		}
	}
